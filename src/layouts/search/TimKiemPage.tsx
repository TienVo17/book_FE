...
import LoiTaiDuLieu from "../products/components/LoiTaiDuLieu";
import KhongCoKetQua from "../products/components/KhongCoKetQua";
...
const TUY_CHON_SAP_XEP: TuyChonSapXep[] = [
...
function TimKiemPage() {
  ...
  const maTheLoai = Number(searchParams.get("maTheLoai")) || 0;
  ...
  const xoaBoLoc = (key: "q" | "giaMin" | "giaMax" | "sort" | "maTheLoai") => {
    capNhatThamSo({ [key]: undefined });
  };

  const boLocDangApDung: { key: "q" | "giaMin" | "giaMax" | "sort" | "maTheLoai"; nhan: string }[] = [];
  ...
  if (maTheLoai > 0) boLocDangApDung.push({ key: "maTheLoai", nhan: "Đã lọc theo thể loại" });
  ...
  return (
    <div className="container py-4">
      ...
      {!dangTai && !loi && danhSach.length === 0 && (
        <KhongCoKetQua thongDiep="Không tìm thấy sách phù hợp. Hãy thử bỏ bớt bộ lọc hoặc đổi từ khóa." />
      )}
