...
differs per environment. `npm run build` runs `scripts/write-robots-sitemap.js`,
which rewrites the `Sitemap:` line in `build/robots.txt` using the same
`REACT_APP_API_BASE_URL` the bundle was compiled with:
